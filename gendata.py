...
# faster function for generating pair of first order binary seqs
def genbasicseqs(N=1000,params=None,plot=True,plot_samples=-1):
    # if no params specified do this simple tree
    if params is None:
        print("Must specify parameters")
        raise Exception
    ...
    for n in range(N):
        px = xparams[x[-1],y[-1]]
        py = yparams[x[-1],y[-1]]
        x.append(np.random.binomial(1,px[0]))
        y.append(np.random.binomial(1,py[0]))
        pxs[:,n] = np.asarray(px)
    # plot the data
    if plot:
        fig,[ax1,ax2] = plt.subplots(2,1,figsize=(15,6))
        ax1.plot(x[1:],'ro',label='Sequence')
        ax1.plot(y[1:],'bo',markersize=1,label='Side Sequence')
        ax1.set_title('Generated Sequences')
        ax1.legend()
        if plot_samples > 0:
            ax1.set_xlim([N-plot_samples,N])
...
# faster function for generating pair of first order binary seqs
def genfirstorder(N=1000,params=None,plot=True,plot_samples=-1,numx=3):
    # if no params specified do this simple tree
    if params is None:
        print("Must specify parameters")
        raise Exception
